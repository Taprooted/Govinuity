...
const SEED_DECISIONS = [
  // Proposed — these land in the review queue
  {
    ...
    reversibility: "Low — data loss is rarely recoverable.",
    source_type: "harvest",
    created_at: now,
    updated_at: now,
  },
  {
    ...
    transfer_tier: "always",
    confidence: 0.82,
    summary_for_human: "Standardise on Postgres for new persistent data stores.",
    why_surfaced: "Recurred across three project setup sessions — each time Postgres was chosen after deliberation.",
    reversibility: "Medium — migration is possible but expensive.",
    source_type: "harvest",
    created_at: now,
    updated_at: now,
  },
  // Approved — these are in the active decision log
  {
    ...
    reversibility: "N/A — a constraint, not a reversible decision.",
    ratified_by: "human",
    source_type: "manual",
    created_at: now,
    updated_at: now,
  },
  {
    ...
    reversibility: "Low — clients build on this; changing it is a breaking change.",
    ratified_by: "human",
    source_type: "harvest",
    created_at: now,
    updated_at: now,
  ...
  const insert = db.transaction(() => {
    for (const d of SEED_DECISIONS) {
      stmt.run({
        ...
        ratified_by: d.ratified_by ?? null,
